...
MImplementRTTI(ZActor2, ZCharacterObject);

///////////////////////////////////////////////////////////////////////
ZActor2::ZActor2() : ZCharacterObject(), m_nFlags(0), m_pNPCInfo(NULL), m_pModule_Skills(NULL), m_fSpeed(0.0f), m_pBrain(NULL)
{
...
//	RegisterModules();
	...
	m_pNetworkAgent = new ZActor2Network;	//todok 팩토리에서 생성해서 넣어주어야 한다

	m_nDamageCount = 0;
}

//void ZActor2::RegisterModules()
//{
...
void ZActor2::InitProperty() 
{
}

void ZActor2::InitStatus() 
{
	...
	SetClientNPCInfoFromServerNPCInfo2( m_pNPCInfo, pQuestNPCInfo );
...
void ZActor2::SetMyControl(bool bMyControl)
{
...
bool ZActor2::IsDieAnimationDone()
{
...
void ZActor2::TestControl(float fDelta)
{
...
void ZActor2::OnUpdate(float fDelta)
{
	if(m_pVMesh) {
	...
	if (CheckFlag(AF_MY_CONTROL))
	{
		...
		{
			__BP(60,"ZActor2::OnUpdate::ProcessAI");
			if(isThinkAble())
				ProcessAI(fDelta);
			__EP(60);
		}
...
void ZActor2::InitFromNPCType(MQUEST_NPC nNPCType)
{
	...
	SetClientNPCInfoFromServerNPCInfo2( m_pNPCInfo, pNPCInfo );
	...
	m_fSpeed = ZBrain2::MakeSpeed(m_pNPCInfo->fSpeed);
	...
	// brain 세팅
	m_pBrain = ZBrain2::CreateBrain(nNPCType);
	m_pBrain->Init(this);

	_ASSERT(m_pNPCInfo != NULL);
}

void ZActor2::InitMesh(char* szMeshName, MQUEST_NPC nNPCType)
{
	// for test
	RMesh* pMesh;
	...
	{
		_ASSERT(0);
		mlog("ZActor2::InitMesh() -  원하는 모델을 찾을수 없음\n");
		return;
	}

	int nVMID = ZGetGame()->m_VisualMeshMgr.Add(pMesh);
	if(nVMID==-1) mlog("ZActor2::InitMesh() - 캐릭터 생성 실패\n");
...
void ZActor2::UpdateHeight(float fDelta)
{
...
void ZActor2::UpdatePosition(float fDelta)
{
...
/*
void ZActor2::Input(AI_INPUT_SET nInput)
{
...
/*
bool ZActor2::SetBehaviorState(AI_BEHAVIOR_STATE nBehaviorState)
{
...
void ZActor2::OnBehaviorEnter(AI_BEHAVIOR_STATE nBehaviorState)
{

}

void ZActor2::OnBehaviorExit(AI_BEHAVIOR_STATE nBehaviorState)
{

}
*/

void ZActor2::OnBlast(rvector &dir)
{
...
void ZActor2::OnBlastDagger(rvector &dir,rvector& pos)
{
	if (!CheckFlag(AF_MY_CONTROL)) return;
...
void ZActor2::ProcessAI(float fDelta)
{
...
ZActor2* ZActor2::CreateActor(MQUEST_NPC nNPCType, bool bForceCollRadius35)
{
	ZActor2* pNewActor=NULL;

	pNewActor = new ZActor2();

	if (pNewActor)
	{
		pNewActor->InitFromNPCType(nNPCType);
...
bool ZActor2::ProcessMotion(float fDelta)
{
...
void ZActor2::ProcessMovement(float fDelta)
{
...
void ZActor2::RunTo(rvector& dir)
{
	dir.z = 0.0f;
	Normalize(dir);
...
void ZActor2::Stop(bool bWithAniStop)
{
	SetVelocity(0,0,0);
...
void ZActor2::RotateTo(rvector& dir)
{
	SetDirection(dir);
	m_Animation.Input(ZA_INPUT_ROTATE);
}

void ZActor2::Attack_Melee()
{
...
void ZActor2::Attack_Range(rvector& dir)
{
...
ZOBJECTHITTEST ZActor2::HitTest( const rvector& origin, const rvector& to, float fTime, rvector *pOutPos )
{
...
//void ZActor2::OnDamage(int damage, float fRatio)
//{
...
void ZActor2::OnDamaged(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	// 맞는 사운드
...
void ZActor2::OnKnockback(rvector& dir, float fForce)
{
...
void ZActor2::CheckDead(float fDelta)
{
	if (!CheckFlag(AF_MY_CONTROL)) return;
...
bool ZActor2::isThinkAble()// 길찾기와 상태변경이 불가능한 경우
{
	if (!CheckFlag(AF_MY_CONTROL))
		return false;
...
void ZActor2::OnPeerDie(MUID& uidKiller)
{
...
void ZActor2::OnTaskFinishedCallback(ZActor2* pActor, ZTASK_ID nLastID)
{
	if (pActor) pActor->OnTaskFinished(nLastID);
}

void ZActor2::OnTaskFinished(ZTASK_ID nLastID)
{
	if (m_pBrain) m_pBrain->OnBody_OnTaskFinished(nLastID);
}


bool ZActor2::CanSee(ZObject* pTarget)
{
	// 시야에 보이는지 확인
...
bool ZActor2::CanAttackRange(ZObject* pTarget)
{
	ZPICKINFO pickinfo;
...
bool ZActor2::CanAttackMelee(ZObject* pTarget, ZSkillDesc *pSkillDesc)
{
	if (pSkillDesc == NULL)
	{
...
void ZActor2::OnNeglect( int nNum)
{
	if ( nNum == 1)
...
//todok zruleQuestchallenge 쪽에서 이쪽으로 커맨드를 던져주어야 한다
void ZActor2::InputBasicInfo(ZBasicInfo* pni, BYTE anistate)
{
	if (!CheckFlag(AF_MY_CONTROL))
	{
...
void ZActor2::InputBossHpAp( float fHp, float fAp )
{
	if (!CheckFlag(AF_MY_CONTROL))
	{
