...
ZBrain2::ZBrain2() : m_pBody(NULL), m_uidTarget( MUID( 0, 0))
{
	ResetStuckInState();
...
ZBrain2* ZBrain2::CreateBrain( MQUEST_NPC nNPCType)
{
	return new ZBrain2();
}


ZObject* ZBrain2::GetTarget()
{
#ifdef _DEBUG
	// 혼자서 AI 테스트할 경우
...
float ZBrain2::MakePathFindingUpdateTime( char nIntelligence)
{
...
float ZBrain2::MakeAttackUpdateTime( char nAgility)
{
	MQuestNPCGlobalAIValue* pGlobalAIValue = ZGetQuest()->GetNPCCatalogue()->GetGlobalAIValue();
...
float ZBrain2::MakeDefaultAttackCoolTime()
{
...
float ZBrain2::MakeSpeed( float fSpeed)
{
	MQuestNPCGlobalAIValue* pGlobalAIValue = ZGetQuest()->GetNPCCatalogue()->GetGlobalAIValue();
...
void ZBrain2::MakeNeglectUpdateTime()
{
	m_dwNeglectTimer = timeGetTime() + 5500;
}


void ZBrain2::Init( ZActor2* pBody)
{
	m_pBody = pBody;
	...
	if ( m_pBody->GetNPCInfo())
	{
		float fDefaultPathFindingUpdateTime		= ZBrain2::MakePathFindingUpdateTime( m_pBody->GetNPCInfo()->nIntelligence);
		float fAttackUpdateTime					= ZBrain2::MakeAttackUpdateTime( m_pBody->GetNPCInfo()->nAgility);
...
void ZBrain2::Think( float fDelta)
{
...
void ZBrain2::ProcessAttack( float fDelta)
{
	...
	{	// (실제 근접타격 판정할때 벽체크하는 함수) CheckWall로 타겟과 나 사이에 장애물이 없는지 확인- 안그러면 기둥 뒤에서 계속 헛방친다
		...
		ZTask2* pNew = ZTaskManager2::CreateAttackMelee( m_pBody);
	...
	if ( GetUseableSkill( &nSkill, &uidTarget, &targetPosition)) 
	{
		...
		if ( m_pBody->CanSee( GetTarget()))
		{
			...
			ZTask2* pNew = ZTaskManager2::CreateSkill( m_pBody, nSkill, uidTarget, targetPosition);
...
bool ZBrain2::GetUseableSkill( int *pnSkill, MUID *puidTarget, rvector *pTargetPosition)
{
	// Get skill module
...
void ZBrain2::ProcessBuildPath( float fDelta)
{
	// Update timer
...
void ZBrain2::PushWayPointsToTask()
{
	// Push task
	...
	for ( list<rvector>::iterator itor = m_WayPointList.begin();  itor != m_WayPointList.end();  ++itor)
	{
		...
		ZTask2* pNew = ZTaskManager2::CreateMoveToPos( m_pBody, (*itor), bChained);
		m_pBody->m_TaskManager.Push( pNew);
...
void ZBrain2::AdjustWayPointWithBound(list<rvector>& wayPointList, RNavigationMesh* pNavMesh)
{
...
bool ZBrain2::EscapeFromStuckIn(list<rvector>& wayPointList)
{
...
void ZBrain2::ResetStuckInState()
{
	rvector pos;
...
void ZBrain2::ResetStuckInStateForWarp()
{
	rvector pos;
	if (m_pBody) pos = m_pBody->GetPosition();
	else	pos.x = pos.y = pos.z = 0;
...
void ZBrain2::OnBody_AnimEnter(ZA_ANIM_STATE nAnimState)
{
}


void ZBrain2::OnBody_AnimExit(ZA_ANIM_STATE nAnimState)
{
}


void ZBrain2::OnBody_CollisionWall()
{
}


void ZBrain2::OnBody_OnTaskFinished( ZTASK_ID nLastID)
{
