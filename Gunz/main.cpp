...
int PASCAL WinMain(HINSTANCE this_inst, HINSTANCE prev_inst, LPSTR cmdline, int cmdshow)
{
	...
	//_CrtSetBreakAlloc(4194);

#ifdef _TDD
	if (0 != TestMain(__argc, __argv)) _ASSERT(0);
	// 여기서 ASSERT 실패가 뜬 경우, 출력창에서 test error 메시지를 찾아 더블클릭하면 실패한 테스트를 볼 수 있습니다.

	// 그냥 return 0; 해버리면 메모리풀 해제가 실행되지 않아 leak이 납니다. 일단 임시로 수동 해제 했습니다.
	// 이런 수동 해제가 많이 필요해지면 리팩토링을 해야겠습니다.
	ZBasicInfoItem::Release();	
	
	// 다음 실행인수가 있으면 테스트들만 수행하고 끝냅니다
	if (0==stricmp(cmdline, "/UnitTest"))
		return 0;

	// test suite 코드에서 발생한 메모리 릭을 찾고 싶으면 아래 행을 주석 풀고 실행하세요
	//return 0;
#endif
